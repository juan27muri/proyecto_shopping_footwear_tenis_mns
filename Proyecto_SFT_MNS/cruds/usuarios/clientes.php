...
WHERE usuarios.rol_id = 2 $filtro
...
if (isset($_POST['verificar_contraseña'])) {
    ...
    if ($admin && password_verify($adminPassword, $admin->contraseña_usuario)) {
        ...
        <script>
            ...
            if (confirmDelete) {
                ...
                .then(result => {
                    ...
                    window.location.href = 'clientes.php'; // Redirigir después de la eliminación
                });
            }
        </script>
        <?php
        ...
        echo '<script>alert("Contraseña incorrecta"); window.location.href = "clientes.php";</script>';
    }
    exit();
}
...
if (isset($_POST['confirmar_eliminar'])) {
    ...
    try {
        ...
        // Eliminar los registros de soporte asociados al usuario
        $sqlEliminarSoporte = "DELETE FROM soporte WHERE usuario_id = :id_usuario";
        $stmtEliminarSoporte = $base->prepare($sqlEliminarSoporte);
        $stmtEliminarSoporte->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Eliminar los detalles de los pedidos asociados al usuario
        $sqlEliminarDetallesPedidos = "
            DELETE detalles_pedido FROM detalles_pedido
            INNER JOIN pedidos ON detalles_pedido.pedido_id = pedidos.numero_pedido
            WHERE pedidos.usuario_id = :id_usuario";
        $stmtEliminarDetallesPedidos = $base->prepare($sqlEliminarDetallesPedidos);
        $stmtEliminarDetallesPedidos->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Eliminar los pedidos asociados al usuario
        $sqlEliminarPedidos = "DELETE FROM pedidos WHERE usuario_id = :id_usuario";
        $stmtEliminarPedidos = $base->prepare($sqlEliminarPedidos);
        $stmtEliminarPedidos->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Eliminar los detalles de los carritos asociados al usuario
        $sqlEliminarDetallesCarrito = "
            DELETE detalles_carrito FROM detalles_carrito
            INNER JOIN carritos ON detalles_carrito.carrito_id = carritos.id_carrito
            WHERE carritos.usuario_id = :id_usuario";
        $stmtEliminarDetallesCarrito = $base->prepare($sqlEliminarDetallesCarrito);
        $stmtEliminarDetallesCarrito->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Eliminar los carritos asociados al usuario
        $sqlEliminarCarritos = "DELETE FROM carritos WHERE usuario_id = :id_usuario";
        $stmtEliminarCarritos = $base->prepare($sqlEliminarCarritos);
        $stmtEliminarCarritos->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Eliminar el usuario
        $sqlEliminarUsuario = "DELETE FROM usuarios WHERE id_usuario = :id_usuario";
        $stmtEliminarUsuario = $base->prepare($sqlEliminarUsuario);
        $stmtEliminarUsuario->execute(array(":id_usuario" => $idUsuarioEliminar));

        // Confirmar la transacción
        $base->commit();

        echo "El usuario y todas las entidades relacionadas han sido eliminados correctamente.";
    } catch (Exception $e) {
    ...
    <div class="admin-container">
        ...
        <main id="main-content">
            <div id="usuarios" class="section active">
                <h1><span class="color">Gestionar Clientes</span></h1>
                <div class="crud-container">
                <a href="register_Cliente.php" class="buttonA">Agregar Cliente</a>
                <br><br>
                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="get" autocomplete="off">
                        <input type="text" name="buscar" placeholder="Buscar Cliente" value="<?php echo isset($_GET['buscar']) ? htmlspecialchars($_GET['buscar']) : ''; ?>">
